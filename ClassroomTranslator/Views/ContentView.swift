...
struct ContentView: View {
    ...
    private var mainContent: some View {
        VStack(spacing: 16) {
            if recentSegments.isEmpty && currentEnglish.isEmpty {
                emptyStateView
            } else {
                transcriptView
            }
    ...
    private var transcriptView: some View {
        ScrollView {
            LazyVStack(alignment: .leading, spacing: 12) {
                ForEach(Array(recentSegments.enumerated()), id: \.offset) { index, segment in
                ...
                if !currentEnglish.isEmpty {
                    VStack(alignment: .leading, spacing: 4) {
                        ...
                        Text(currentChinese.isEmpty ? "Translating..." : currentChinese)
                            .font(.system(size: 13))
    ...
    private func setupSubtitleWindow() {
        ...
        speechManager.onSegmentRecognized = { [weak subtitleWindowController] text, isFinal in
            Task { @MainActor in
                if isFinal {
                    ...
                    subtitleWindowController?.updateSegments(recentSegments)
                    ...
                    currentEnglish = text
                    subtitleWindowController?.updateSegments(recentSegments, currentText: text)
                }
            }
        }
    }
    ...
    private func toggleOverlay() {
        if subtitleWindowController?.window?.isVisible == true {
            ...
            subtitleWindowController?.showWindow()
        }
    }
}
